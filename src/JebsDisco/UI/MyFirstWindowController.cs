...
using JebsDisco.Unity.Runtime;
using UitkForKsp2.API;
using UnityEngine;
using UnityEngine.UIElements;

namespace JebsDisco.UI;
...
public class MyFirstWindowController : MonoBehaviour
{
    ...
    public bool IsWindowOpen
    {
        ...
        set
        {
            ...
            GameObject.Find(JebsDiscoPlugin.ToolbarFlightButtonID)
                ?.GetComponent<UIValue_WriteBool_Toggle>()
                ?.SetValue(value);

            // Update the OAB AppBar button state
            GameObject.Find(JebsDiscoPlugin.ToolbarOabButtonID)
                ?.GetComponent<UIValue_WriteBool_Toggle>()
                ?.SetValue(value);
        }
    }
